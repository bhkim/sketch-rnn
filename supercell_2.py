...
class LSTMCell(tf.contrib.rnn.RNNCell): 
    ...
    def __init__(self, num_units, forget_bias=1.0, use_layer_norm=False,
        use_recurrent_dropout=False, dropout_keep_prob=0.90, state_is_tuple=True):
...
class MultiRNNCellWithSkipConn(core_rnn_cell.RNNCell):    # from tensorflow.models.rnn.rnn_cell import RNNCell
        ...
        with vs.variable_scope(scope or "multi_rnn_cell_with_skipconn"):    
            ...
            for i, cell in enumerate(self._cells):
                with vs.variable_scope("cell_%d" % i):
                    ...
                    cur_inp, new_state = cell(tf.concat([inputs, first_layer_input], 1), cur_state) #cell(cur_inp, cur_state)
