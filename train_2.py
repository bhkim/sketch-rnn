...
def main():
    ...
    parser.add_argument('--skipconn', type=bool, default=True,
...
def email_report(mail_subject, fileToSend):
    # send reports via e-mail
    msg = MIMEMultipart()
    senderEmail = 'abc@foo.com'
    receiverEmail = 'efg@foo.com'
...
def train(args):
    ...
    with tf.Session(config=config) as sess:
        ...
        def save_model(tot_loss, shape_loss, pen_loss):  # modified by bhkim to get three losses as parameters: 2016-11-17
        ...
        for e in xrange(args.num_epochs):
            ...
            while data_loader.epoch_finished == False:
                ...
                if train_loss < min_train_loss:
                    savefilename = save_model(train_loss, shape_loss, pen_loss)
                    min_train_loss = train_loss
                    
                    filesToSend = '{}'.format(savefilename)
                    mail_subject = '%s - layernorm ep%d, data: %s, cost = %.2f (%.2f + %.4f)' % (args.model, e, args.dataset_name, train_loss, shape_loss, pen_loss)
